...
const getDropTarget = (e: HTMLElement) => {
    const start = e;
    while (e) {
        if (e.hasAttribute("data-upload-requested")) {
            return e;
        }
        e = e.parentElement;
    }
...
const dragEnter = (e: DragEvent) => {
    ...
    getDropTarget(e.currentTarget as HTMLElement).setAttribute("data-drop-enabled", "1");
};
...
const dragLeave = (e: DragEvent) => {
    ...
    getDropTarget(e.currentTarget as HTMLElement).removeAttribute("data-drop-enabled");
};
...
const eventDrop = (e: DragEvent) => {
    ...
    e.preventDefault();
    const currentTarget = getDropTarget(e.currentTarget as HTMLElement);
    ...
    const fileHandlesPromises = [...e.dataTransfer.items as any]
        ...
        .map((item) =>
            ...
            ? item.getAsFileSystemHandle().then((f) => f ?? item.getAsFile())
